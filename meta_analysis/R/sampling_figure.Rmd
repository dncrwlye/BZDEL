...
x<-ggplot() +
  #geom_rect(data=rect_sampling_point_unique, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, fill='Multiple Sampling Events, Multiple Estimates'), color=NA, alpha=0.2) +
  #geom_rect(data=singe_time_point, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, fill='Single Sampling Event, Single Estimate'), color=NA, alpha=0.4) +
  geom_rect(data=rect_sampling_point_long, aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, fill='Multiple Sampling Events, Single Estimate'), color=NA, alpha=0.4) +
  geom_line(data=smooth_yt, aes(y=V1, x=x-20)) +
  ylab('seroprevalence') +
  xlab('julian days') +
...
ggsave(filename = '/Users/buckcrowley/Desktop/BDEL/BZDEL/meta_analysis/sampling_plot.pdf', plot=x)
...
paper_breakdown_table_alternative <- matrix(c(
                                  ...
                                  "Multiple Sampling Events, Multiple Estimates", nrow(explicit_longitudinal), round(nrow(explicit_longitudinal)/nrow(seroprevalence_total),3)*100, "", "",
                                  "Single Sampling Event, Single Estimate", nrow(single_time_points_but_decent_range), round(nrow(single_time_points_but_decent_range)/nrow(seroprevalence_total),3)*100, "", "",
                                  "Multiple Sampling Events, Single Estimate", nrow(pooled_estimates_just_horrible), round(nrow(pooled_estimates_just_horrible)/nrow(seroprevalence_total),3)*100, round(pooled_estimates_just_horrible_mean_std$mean,1), round(pooled_estimates_just_horrible_mean_std$sd,1)
                                  )
                                  , byrow=TRUE,ncol=5)
kable(paper_breakdown_table, align = 'l', col.names = c('', 'Count', '%', 'Mean', 'Std. Dev.'))
...
x<- seroprevalence %>%
  ...
  select(c(species, title)) %>%
  unique() %>%
  group_by(species) %>%
  summarise(n=n()) %>%
  mutate(n_cat = ifelse(n==1, 1, 0))
