...
colfcn <- function(n) return(c("#CC00FFFF", "#FF0099FF"))
pf.tree <- pf.tree(pf, lwd=1, factors = 1:2, color.fcn=colfcn, branch.length = "none")
...
pf.tree$ggplot +
  ...
  geom_segment(inherit.aes= TRUE, data= d,aes(x=x,y=y,xend=xend,yend=yend, size= Data$log_effort, col = 'red'))

geom_segment(aes(x = 2, y = 15, xend = 3, yend = 15))


Legend <- pf.tree$legend
Legend$names <- names.storage[c(1:2)]
