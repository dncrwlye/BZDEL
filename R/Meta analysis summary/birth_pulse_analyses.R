...
seroprevalence_single_time_point <- seroprevalence_x_final %>%
    ...
    group_by(month, virus, species, coordinate_box,  methodology, group) %>%
...
seroprevalence_unclear_time_point <- seroprevalence_x_final %>%
    ...
    unite(group, c(title, age_class, last_name_of_first_author, year), sep = ", ") %>%
    group_by(month_1, month_2, virus, species, coordinate_box, methodology, group) %>%
...
seroprevalence_graph_inner_join <- inner_join(seroprevalence_graph, Bat_Birth_Pulse_Data)


seroprevalence_graph_full_join <- full_join(seroprevalence_graph, Bat_Birth_Pulse_Data)
