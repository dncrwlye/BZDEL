...
seroprevalence_henipavirus_single_time_point <- filovirus.seroprevalence %>%
    ...
    group_by(month, age_class, title, species, country, last_name_of_first_author, global_location) %>%
...
seroprevalence_henipavirus_unclear_time_point <- filovirus.seroprevalence %>%
    ...
    group_by(month_1, month_2, age_class, species, country, title, last_name_of_first_author, global_location) %>%
  ...
  x <- seroprevalence_henipavirus_unclear_time_point[,c(2:13)]
  y <-seroprevalence_henipavirus_unclear_time_point[,c(1,3:13)]
...
x<-as.data.frame(table(seroprevalence_graph$species, seroprevalence_graph$country)) %>%
  filter(Freq != 0)
...
seroprevalence_graph %>%
  ...
  ungroup() %>%
  ggplot() +
  geom_point(aes(x= month.dc.birthpulse, y= seroprevalence_per, colour = species, group = species)) +
  geom_line(lwd=1, aes(x= month.dc.birthpulse, y=seroprevalence_per, colour = species, group = species)) +
